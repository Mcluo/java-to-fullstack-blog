...
export default function FeedConfigManager() {
  ...
  const [form, setForm] = useState({ name: '', url: '', category: '', maxItems: 10 })
  ...
  function handleEdit(feed: FeedSource) {
    ...
    setForm({ name: feed.name, url: feed.url, category: feed.category, maxItems: feed.maxItems })
    setDiscovery(null)
    setShowForm(true)
  }

  function resetForm() {
    setForm({ name: '', url: '', category: '', maxItems: 10 })
    setEditingId(null)
    setShowForm(false)
    setDiscovery(null)
  }
        ...
        <form onSubmit={handleSubmit} className="bg-gray-50 rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="输入任意 URL — 博客地址、公众号、GitHub、掘金、知乎..."
                value={form.url}
